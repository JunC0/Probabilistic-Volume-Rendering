...
"                    cur_color=cur_color+t_color*t_alpha*t_alpha*(1-alpha);\n"
"                    alpha=alpha+(1-alpha)*t_alpha*t_alpha*t_alpha;\n"
"                    if(alpha>0.99)break;\n"
"                }\n"
"            }\n"
"        }\n"
...
void GLWidget::initializeGL()
{
    ...
    for(int i=0;i<10;i++){
        ...
        glBindTexture(GL_TEXTURE_1D,color_tex[i]);
        glTexParameteri(GL_TEXTURE_1D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
        glTexParameteri(GL_TEXTURE_1D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
        glTexParameteri(GL_TEXTURE_1D,GL_TEXTURE_WRAP_S,GL_CLAMP_TO_EDGE);
        glTexImage1D(GL_TEXTURE_1D,0,GL_RGBA,256,0,GL_RGBA,GL_FLOAT,color_table[i]);
        glActiveTexture(GL_TEXTURE3+i);
        glUniform1i(con_color_table[i],3+i);
        glBindTexture(GL_TEXTURE_1D,color_tex[i]);
...
void GLWidget::ReadColorTable(){
    ...
    char t[20];
    for(int j=0;j<11;j++){
        sprintf(t,"color_table%d.txt",j);
        f=fopen(t,"r");
        for(int i=0;i<256;i++){
            fscanf(f,"%f %f %f %f",&r,&g,&b,&a);
            color_table[j][i]=QVector4D(r,g,b,a);
        }
        fclose(f);

        glBindTexture(GL_TEXTURE_1D,color_tex[j]);
        glTexSubImage1D(GL_TEXTURE_1D,0,0,256,GL_RGBA,GL_FLOAT,color_table[j]);
    }
}
...
void GLWidget::paintGL()
{
    ...
    box_min=const_box_min*scale;
    box_max=const_box_max*scale;
